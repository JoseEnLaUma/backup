...
sizeValue x | a == Nothing = 0
            | otherwise = 1

-- | = Exercise d - size
size :: Trie a -> Int
size Empty = 0
size Node a (b c) = sizeValue b + sum (map size (D.values c))
...
update (Node r dict) c child = Node r (D.insert c child dict)

-- | = Exercise i - insert
insert :: String -> a -> Trie a -> Trie a
insert [] v Empty = Node (Just v) (D.Empty)
insert [] v (Node r d) = Node (Just v) d
insert (c:cs) v trie = update trie c (insert cs v (childOf c trie))
